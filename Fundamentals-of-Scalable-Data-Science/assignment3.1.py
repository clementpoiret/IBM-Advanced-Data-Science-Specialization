...
def minTemperature(df,spark):
    ...
    #could have used df.agg({"temperature": "min"}).collect() too
...
def sdTemperature(df,spark):
    ...
    #temp = df.select("temperature").rdd.map(lambda x : x["temperature"])
    #temp = temp.filter(lambda x : x is not None).filter(lambda x : x is not "")
    #n = temp.count()
    #mean = temp.sum()/n
...
def skewTemperature(df,spark):    
    temp = df.select("temperature").rdd.map(lambda x : x["temperature"]).filter(lambda x : x is not None).filter(lambda x : x is not "")
    n = temp.count()
    mean = meanTemperature(df,spark)
    sd = sdTemperature(df,spark)
    skewness = temp.map(lambda x : pow(x-mean, 3)/pow(sd, 3)).sum()/n
    #could have done return spark.sql("SELECT SKEWNESS(temperature) as skewtemp from washing").first().skewtemp
    return skewness
...
def kurtosisTemperature(df,spark):    
    ...
    mean = meanTemperature(df,spark)
    sd = sdTemperature(df,spark)
    kurtosis = temp.map(lambda x : pow(x-mean, 4)/pow(sd, 4)).sum()/n
    return kurtosis
